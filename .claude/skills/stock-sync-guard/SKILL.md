...
| **Tonsa is the only supplier** | Tonsa is blocked because RP's price for them is wrong — a pricing problem, not a stock problem. The part is in stock. | Note `CANT ROUTE - TONSA BLOCKED (RP PRICING) - NEEDS ADAM`, alert via `alert.js`. **No quantity change, no buyer message.** |
...
| **Capital Chevrolet of Wake Forest is the only supplier** | Barred by Adam 2026-09-10. Like Griffin: a vendor problem, not a stock problem — the part is in stock. | Stop. Alert Adam with `alert.js`, note `CANT ROUTE - CAPITAL CHEVROLET BARRED - ADAM NOTIFIED`. **No quantity change, no buyer message.** |
| **#1 Cochran Automotive Group is the only supplier** | Barred again 2026-09-17 — a vendor decision, not a stock problem. | Stop. Shared-warehouse modal first (Gideon if another warehouse has it); else alert Adam with `alert.js`, note `CANT ROUTE - COCHRAN ONLY - ADAM NOTIFIED`. **No quantity change, no buyer message.** |
